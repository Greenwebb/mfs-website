...
<head>
  ...
  <title>Add Repayments || Mighty Finance </title>
  <!-- base:css -->
        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title"> Add Loan Repayment</h4>
                  <p class="card-description">
                    Add Loan repayment
                  </p>
                  <form method="post" id="repayment" action="includes/logic/repayment.php" class="forms-sample">
                    ...
                    <div class="row">
                      ...
                      <div class="form-group col-8">
                        <label for="exampleInputName1">Loan Details</label>
                        <div class="input-group">
                          <div class="input-group-prepend">
                            <span class="input-group-text"><em class="mdi mdi-account-box-outline"></em></span>
                          </div>
                          <select required name="lid" id="borrower_search" class="form-control">
                          <option disabled selected>select client</option>

                            <?php $result = mysqli_query($con, "SELECT * FROM clients INNER JOIN  loans ON loans.client_id = clients.id WHERE loans.status = 'approved' ");
...
<option value="$lidd" >  $row[lname] $row[fname]'s  loan of K$row[amount] </option>

_iwe;
}
}else{

  echo "<option>No results in the system</option>";
                      ...
                      <div class="form-group col-4">
                        ...
                        <div class="input-group ">
                          <div class="input-group">
                            ...
                            <input required type="number" class="form-control" id="amount" name="amount" placeholder="amount" aria-label="email">
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <button type="submit" name="repayment" value="repayment" class="btn btn-primary mr-2 btn-lg btn-block">Submit</button>
                  </form>
                </div>
              </div>
            </div>
  ...
  $("#repayment").on('submit', function(event){
		...
		$.ajax({
			url:"includes/logic/repayment.php",
			method:"POST",
			data:formData,
